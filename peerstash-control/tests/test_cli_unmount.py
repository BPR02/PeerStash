...
from peerstash.cli.cmd_unmount import app


def test_cancel_success(mock_setup, runner: CliRunner, mocker: MockerFixture):
    mock_run = mocker.patch("peerstash.cli.cmd_unmount.unmount_task")
...
def test_cancel_value_error(runner: CliRunner, mocker: MockerFixture):
    mocker.patch(
        "peerstash.cli.cmd_unmount.unmount_task",
...
def test_cancel_system_error(runner: CliRunner, mocker: MockerFixture):
    mocker.patch(
        "peerstash.cli.cmd_unmount.unmount_task", side_effect=Exception("Daemon down")
    )
